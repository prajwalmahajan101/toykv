...
**Next: cut `v3.0.0`** (planned, not yet executed — post-merge of `feat/toyraft-v1`). Via `code_assist:release`: CHANGELOG entry (v3.0 = replication + ToyRaft `v1.0.0`) → version bump → annotated `v3.0.0` tag → goreleaser build (4 binaries, reused v1/v2 config) → GitHub release. Then the [v3.x backlog](#v3x-backlog-not-in-committed-v30-scope) opens (compaction on ToyRaft `v2` snapshots; linearizable reads on ReadIndex; sets/sorted-sets/pub-sub on the replicated `Apply` path).
