...
- **Coordinate ToyRaft `v1.0.0`:** ✅ **done** — dependency bumped `rc.2 → v1.0.0` (`go.mod`) once ToyRaft tagged it off this integration. Only adapt: `raft.Node.Propose` now returns a fourth value (the Apply result), which toykv ignores (keeps its per-index reply registry). Both dogfood asks resolved upstream (`raft.Node.NodeID()`; nil-defaulting `inproc.HubConfig.Clock`) — toykv keeps its `MatchIndex` self-filter and HTTP-path `chaosTransport` deliberately.
...
**Done: cut `v3.0.0`** (2026-09-19). CHANGELOG entry (v3.0 = replication + ToyRaft `v1.0.0`) → `serverVersion` bump `2.0.0 → 3.0.0` → annotated `v3.0.0` tag → goreleaser build (4 binaries, reused v1/v2 config) → GitHub release. The [v3.x backlog](#v3x-backlog-not-in-committed-v30-scope) is now open (compaction on ToyRaft `v2` snapshots; linearizable reads on ReadIndex; sets/sorted-sets/pub-sub on the replicated `Apply` path).
...
| M23 | Bench + dogfood report + polish + v3.0.0 | ✅ | _feat/release-v3, feat/toyraft-v1_ | `v3.0.0` |
