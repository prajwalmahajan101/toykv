...
| 0021 | Replication acknowledgement, INFO replication & cluster OTel | After M21 lands | M21 |
...
**Budget (v3, M18–M23): 18 written** — the v2 set (below) plus 0018, 0019, 0020, 0021. Each of the four v3 ADRs clears the "real architectural decision" bar: 0018 the Raft embedding + command-envelope + `StateMachine` seam (the mutate chokepoint becomes `Propose → Apply`; AOF demoted to local durability, Raft log as replication source); 0019 the cluster mode contract (HTTP peer transport, file-backed log layout, and — amended at M23 — the plaintext-peer-transport security boundary the `checkRaftBind` guard enforces); 0020 write redirection + the non-linearizable read model (`NOTLEADER` hint, `READONLY` opt-in); 0021 the replication-ack + telemetry model (`WAIT` truthfulness against `MatchIndex`, `INFO replication`). **M22 (TUI v3) and M23 (release) added no ADR** — consumer/release milestones per the M13/M14 precedent (the raft-bind guard is an M23 amendment to 0019, not a new decision).
