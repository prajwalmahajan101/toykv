...
// is non-empty) GET <focused>, then returns a refreshMsg. All client
// calls run on the tea.Cmd goroutine; the client's mutex serialises
// them naturally.
func fetchRefresh(c Doer, pattern, focused string) tea.Cmd {
	if pattern == "" {
		pattern = "*"
	}
	return func() tea.Msg {
		...
		if err != nil {
			return refreshMsg{err: err.Error(), latency: time.Since(start)}
		}
		...
		msg := refreshMsg{keys: infos, dbsize: dbsize, latency: time.Since(start)}
		if focused != "" {
			v, err := c.Do("GET", focused)
			if err != nil {
				return refreshMsg{
					keys: infos, dbsize: dbsize,
					err: err.Error(), latency: time.Since(start),
				}
			}
			msg.value = v
			msg.hasVal = true
